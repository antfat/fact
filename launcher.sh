...
apt-get update || true
apt-get install -y \
  wget curl ca-certificates bash coreutils procps findutils sed grep rsync \
...
CADO_CLIENT="$(find /usr /opt /root/launcher -name "cado-nfs-client.py" -type f 2>/dev/null | head -1 || true)"

if [ -z "$CADO_CLIENT" ]; then
  log "cado-nfs-client.py не найден. Пробую установить cado-nfs"
  apt-get update || true
...
if [ -n "$CADO_CLIENT" ]; then
  ...
  chmod +x /cado-nfs/cado-nfs-client.py || true
else
  log "ВНИМАНИЕ: cado-nfs-client.py не найден"
fi
